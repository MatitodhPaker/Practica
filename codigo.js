...
async function entrenamiento(model, inputs, labels) {
    model.compile({
...
async function run() {
    ...
    modelo= crearModelo();
    const tensorData = convetirDatos(data);
    const {entradas, etiquetas}= tensorData;
    await entrenamiento(modelo,entradas,etiquetas);
}
run();